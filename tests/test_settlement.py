...
def test_thirty_360_treats_every_month_as_thirty_days():
    ...
    assert np.isclose(day_count_fraction(date(2023, 2, 15), date(2023, 3, 15), "30/360"), 30 / 360)
...
def test_thirty_360_collapses_a_31st_end_date_only_after_start_adjusts():
    ...
    assert np.isclose(day_count_fraction(date(2024, 1, 31), date(2024, 3, 31), "30/360"), 60 / 360)
    # Start on a 30th, end on a 31st: end still collapses to 30.
    assert np.isclose(day_count_fraction(date(2024, 1, 30), date(2024, 3, 31), "30/360"), 60 / 360)
    # Start mid-month: the end 31st does NOT collapse, so Jan 15 -> Mar 31
    # is 30 + 16 = 76 days, not 75.
    assert np.isclose(day_count_fraction(date(2024, 1, 15), date(2024, 3, 31), "30/360"), 76 / 360)
...
def test_act_act_uses_366_in_a_leap_year():
    ...
    assert np.isclose(day_count_fraction(date(2023, 1, 1), date(2023, 3, 1), "ACT/ACT"), 59 / 365)
...
def test_the_convention_actually_changes_the_answer():
    ...
    by_convention = {
        c: accrued_interest(convention=c, **kwargs)
        for c in ("30/360", "ACT/360", "ACT/365", "ACT/ACT")
    }
...
def test_accrued_is_zero_on_a_coupon_date_and_full_coupon_after_one_period():
    ...
    assert np.isclose(accrued_interest(100.0, 0.06, coupon_date, coupon_date, 2, "30/360"), 0.0)
